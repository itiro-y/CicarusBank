...
// ------------------ MODAL DE EDIÇÃO DE AVATAR MODIFICADO ------------------
function AvatarEditorDialog({ open, onClose, onSave }) {
    ...
    // --- FUNÇÃO MODIFICADA PARA CHAMAR O BACKEND REAL ---
    const handleGenerateAI = async () => {
        ...
        try {
            ...
            // A imagem vem como Base64 do backend, pronta para ser usada
            const imageSrc = `data:image/png;base64,${data.imageBase64}`;
            setImgSrc(imageSrc);
            setTab(0); // Leva o usuário para a aba de corte
    ...
    return (
        <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
            ...
            <DialogContent sx={{ p: 0 }}>
                <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                    <Tabs value={tab} onChange={(e, newValue) => setTab(newValue)} centered variant="fullWidth">
                        <Tab icon={<CloudUpload />} label="Fazer Upload" />
                        <Tab icon={<AutoAwesome />} label="Gerar com IA" />
                    </Tabs>
                </Box>
                <Box sx={{ p: 3, minHeight: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {tab === 0 && (
                        <Stack spacing={2} alignItems="center">
                            {!imgSrc && (<Button variant="outlined" onClick={() => fileInputRef.current?.click()} startIcon={<PhotoCamera />}>Selecionar Imagem do Computador</Button>)}
                            <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} hidden />
                            {imgSrc && (<ReactCrop crop={crop} onChange={c => setCrop(c)} onComplete={c => setCompletedCrop(c)} aspect={1} circularCrop> <img ref={imgRef} alt="Crop" src={imgSrc} onLoad={onImageLoad} style={{ maxHeight: '400px' }}/> </ReactCrop>)}
                        </Stack>
                    )}
                    {tab === 1 && (
                        <Stack spacing={2} alignItems="center" sx={{ width: '100%' }}>
                            <Typography variant="body2" color="text.secondary" textAlign="center">Descreva como você quer seu novo avatar.</Typography>
                            <TextField fullWidth multiline rows={2} label="Ex: A tiger with sun glasses (prompt deve ser em inglês)" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
                            <Button variant="contained" onClick={handleGenerateAI} disabled={isGenerating || !prompt} startIcon={isGenerating ? <CircularProgress size={20} color="inherit" /> : <AutoAwesome />}>{isGenerating ? 'Gerando...' : 'Gerar Imagem'}</Button>
            ...
            <DialogActions>
                ...
                <Button onClick={handleSaveCroppedImage} variant="contained" disabled={!completedCrop && !imgSrc}>Salvar</Button>
            </DialogActions>
        </Dialog>
    );
}

// O RESTO DO ARQUIVO (ProfileHeader, ProfilePage, etc.) CONTINUA O MESMO DA RESPOSTA ANTERIOR
...
const SecurityActions = () => { const navigate = useNavigate(); const [isafeCode, setISafeCode] = useState(null); const [countdown, setCountdown] = useState(0); const [loading, setLoading] = useState(false); useEffect(() => { let timer; if (countdown > 0) { timer = setInterval(() => { setCountdown(prev => prev - 1); }, 1000); } else if (countdown === 0 && isafeCode) { setISafeCode(null); localStorage.removeItem('isafeCode'); } return () => clearInterval(timer); }, [countdown, isafeCode]); const handleGenerateISafeCode = async () => { setLoading(true); try { await new Promise(resolve => setTimeout(resolve, 500)); const otp = Math.floor(100000 + Math.random() * 900000).toString(); localStorage.setItem('isafeCode', JSON.stringify({ code: otp, expiry: Date.now() + 300000 })); setISafeCode(otp); setCountdown(300); } catch (error) { console.error(error); } finally { setLoading(false); } }; const formatTime = (seconds) => `${Math.floor(seconds / 60)}:${(seconds % 60).toString().padStart(2, '0')}`; return ( <InfoWidget title="Segurança"> <Stack spacing={2}> <Button variant="contained" startIcon={<Lock />} sx={{ justifyContent: 'flex-start', py: 1.5 }}>Alterar Senha de Acesso</Button> <Button variant="outlined" startIcon={<Shield />} sx={{ justifyContent: 'flex-start', py: 1.5 }} onClick={() => navigate('/connected-devices')}>Gerenciar Dispositivos Conectados</Button> <Button variant="outlined" startIcon={<PhoneAndroid />} sx={{ justifyContent: 'flex-start', py: 1.5 }} onClick={handleGenerateISafeCode} disabled={loading || !!isafeCode}>{loading ? "Gerando..." : isafeCode ? "Código Ativo" : "Gerar Código iSafe"}</Button> {isafeCode && (<Paper elevation={0} sx={{ p: 2, mt: 2, textAlign: 'center', background: theme => theme.palette.action.hover }}><Typography variant="h4" color="primary" sx={{ letterSpacing: '0.5rem', fontWeight: 'bold' }}>{isafeCode}</Typography><Typography variant="body2" color="text.secondary">Válido por: {formatTime(countdown)}</Typography></Paper>)} </Stack> </InfoWidget> ); };
export default function ProfilePage() {
    ...
    const handleSaveProfile = (updatedData) => { setProfile(updatedData); if (updateUser) { updateUser({ name: updatedData.name, email: updatedData.email }); } console.log("Saving to backend:", updatedData); };
            ...
            <Container maxWidth="lg" sx={{ pt: '120px', pb: 4 }}>
                <Stack spacing={3}>
                    ...
                    <Grid container spacing={3}>
                        ...
                        <Grid item xs={12} md={6}> <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.5 }}> <SecurityActions /> </motion.div> </Grid>
                    </Grid>
                </Stack>
